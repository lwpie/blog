...
之前写的 [Huffman 编解码器](https://github.com/lwpie/huffman) 读写超过 4G 文件会报 `basic_filebuf::xsgetn error reading the file: iostream error`，但是 C++ 标准规定大小是 `size_t` 不应该出问题，后来在网友提醒下发现这并不应该出错，但 [Windows API](https://docs.microsoft.com/windows/win32/api/fileapi/nf-fileapi-writefileex) 接受的参数是 `DWORD` 类型也即 `unsigned int`，于是超过 4G 就报错了。
