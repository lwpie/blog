...
作为专注云计算的实验室，一个月前组里只有一台服务器，直接接入校园网，日常也没什么人用。
为了 ~~高效花完本年度科研经费~~ 提供更多组内计算资源，我全程负责了实验室基础设施体系的搭建，在蒙民伟科技南楼地下机房度过无数日夜。
哈利、坏人和 TUNA 的小伙伴们在此过程提供了许多指导，希望这篇文章不仅为自己备忘，也能为你们提供帮助。
...
大部分家用设备（个人电脑、路由器等）使用电口，如图中最左侧的 ETH/BOOT，通过网线连接使用。
五类网线可以提供千兆速率，万兆一般需要六类屏蔽网线，六类也可以进行尝试。

### ii. 光口

光口是对各种光纤端口的总称，如图中右侧四个接口。
网线有效传输距离一般不到百米，最高提供 10/40G 速率；光纤距离以公里计算，速率可达 200/400G，这使机房和交换机中光口更加普遍。
电口可以直接接入网线使用，光口需要插入模块后接入传输介质。
...
2. 生产电模块的厂商常见 TP / Marvell / Broadcom 三种，其余均为贴牌产物：
    ...
    2. TP 专注家用，兼容 10G / 5G / 2.5G / 1G 速率，不建议在机房使用；

    3. Marvell 功率 1.6W，万兆最大距离 30m；Broadcom 功率 2.0W，距离 80m；
...
交换机工作在二层，路由器（网关）工作在三层。
为了帮助理解，可以把交换机看作网口拓展器，接入交换机的设备均可被独立分配校园网 IP，但互相访问的数据包只经过交换机转发；
路由器用于提供 NAT 服务，组建形如 `192.168.1.0/24` 的局域网，并负责到校园网的 NAT 转发。

最近新发售的高端家用路由器（如 XTR10890、AX89X 等）提供各一个万兆光口电口，十分适合在机房使用。
电口接入机房提供的校园网，光口与交换机相连，其余千兆电口用于 IPMI 等电口服务。
...
个人电脑中主板和机箱在服务器被称为准系统，目前接触到超微（Supermicro）、技嘉（Gigabyte）和戴尔（Dell）。
...
IPMI 是通过网络管理设备的接口，是 web 协议的 BIOS 界面，可以避免前往机房使用键盘、显示器调试设备。
常用的服务器准系统均支持 IPMI，在 BIOS 设置或操作系统中设置，常用的工具有 `ipmitool`。
...
使用小容量 SSD 组建 RAID1 作为系统盘，用网络存储在服务器之间共享数据。
NFS 挂载仅取决于 IP，挂载后的权限在服务器间通过 `uid` 对应。
一种解决方式是使用 LDAP 服务统一 `uid`，另一种方式是通过规则将用户 squash 到对应用户，如把所有用户均映射到特定有权限账户。
...
LDAP 是通用的统一认证服务，绝大多数应用均支持 LDAP 认证协议。
古老的传闻是亲自配置 openLDAP 会延毕，等存储服务器安装之后再试着部署。
